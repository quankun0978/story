...
class IndexController extends Controller
{
    ...
    public function readStory($slug)
    {
        $categories = DB::table('categories')->where('status', "active")->get();
        $types = DB::table('types')->where('status', "active")->get();
        $storyBySlug = Story::orderBy('id', 'ASC')->where('slug', $slug)->where('status', "active")->get();
        $share = new Share();
        ...
        $stories = Story::orderBy('id', 'ASC')
            ...
            ->where('id', '!=', $storyBySlug[0]->id)->where('status', "active")
            ->get();
    ...
    public function category(Request $request, $slug, $page)
    {
        $types = DB::table('types')->where('status', "active")->get();
        $categories = DB::table('categories')->where('status', "active")->get();
        $category = Category::orderBy('id', 'ASC')->where('status', "active")->where('slug', $slug)->get();
        $stories = Story::orderBy('id', 'ASC')->where('status', "active")->where('category_id', $category[0]->id)->get();
        $share = new Share();
        $shareComponent = $share->page($request->url(), 'Chia sẻ')
            ->facebook()
            ->twitter()
            ->reddit()
            ->whatsapp()->telegram()->linkedin();
        $storiesByPage = TypeStory::with("story")->where('status', "active")->orderBy('id', 'ASC')->where('type_id', $category[0]->id)->paginate(24, ['*'], 'page', $page);
    ...
    public function type(Request $request, $slug, $page)
    {
        ...
        $types = DB::table('types')->where('status', "active")->get();
        $categories = DB::table('categories')->where('status', "active")->get();
        $type = Type::orderBy('id', 'ASC')->where('status', "active")->where('slug', $slug)->get();
        $stories = TypeStory::with("story")->orderBy('id', 'ASC')->where('status', "active")->where('type_id', $type[0]->id)->get();
        $storiesByPage = TypeStory::with("story")->orderBy('id', 'ASC')->where('status', "active")->where('type_id', $type[0]->id)->paginate(24, ['*'], 'page', $page);
    ...
    public function chapters(Request $request, $slug)
    {
        ...
        $categories = DB::table('categories')->where('status', "active")->get();
        $types = DB::table('types')->where('status', "active")->get();
        $chapterBySlug = Chapter::orderBy('id', 'DESC')->where('status', "active")->where('slug', $slug)->get();
        $chapters = Chapter::orderBy('id', 'DESC')->where('story_id', $chapterBySlug[0]->story_id)->where('status', "active")->get();
        ...
        $chapters = Chapter::orderBy('id', 'ASC')->where('story_id', $chapterBySlug[0]->story_id)->where('status', "active")->get();
